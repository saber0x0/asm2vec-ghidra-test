...
# 用于存储训练参数，如维度 d、初始学习率 initial_alpha、学习率更新间隔 alpha_update_interval、
# 随机游走次数 num_of_rnd_walks、负采样数量 neg_samples、迭代次数 iteration 和工作线程数 jobs。
class Asm2VecParams:
...
# 表示一个滑动窗口，用于在训练过程中遍历指令序列。它提供了访问当前、上一个和下一个指令的方法。
...
# 提供了一个训练上下文，包括函数库 FunctionRepository、参数 Asm2VecParams、学习率 alpha、负采样器 NegativeSampler 和一个锁 _lock 用于同步操作。
# 包含一个嵌套的 Counter 类，用于跟踪训练过程中的不同计数器，例如处理的标记数。
# 提供了创建序列窗口、获取和设置学习率、获取负采样器、创建计数器等方法。
...
# _sigmoid：sigmoid 函数，用于激活。
# _identity：恒等函数，用于条件表达式。
# _dot_sigmoid：计算两个向量的点积后应用 sigmoid 函数。
# _get_inst_repr：获取指令的操作码和参数的向量表示。
...
# 训练一个向量化窗口，计算当前窗口中函数向量和指令向量的梯度，并更新它们。
...
# 使用 SequenceWindow 遍历函数的指令序列，并调用 _train_vectorized 来训练每个窗口。
...
# 接受一个函数库 FunctionRepository 和参数 Asm2VecParams，使用多线程训练库中的每个函数。
...
# 估计给定函数的向量表示，通常用于在训练完成后获取函数的向量表示。
